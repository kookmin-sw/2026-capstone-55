  ...
  if (isCompleted) {
    ...
    completeButtonHtml = `
      <div style="margin-top:24px; text-align:center;">
        <div style="padding:20px; background:var(--color-bg-warm); border-radius:16px;">
          <div style="font-size:2rem; margin-bottom:8px;">📝</div>
          <div style="font-weight:800; font-size:1rem; margin-bottom:6px;">학습 확인 퀴즈</div>
          <p style="font-size:0.85rem; color:var(--color-text-muted); margin-bottom:16px;">퀴즈를 시작하면 본문이 가려져요.<br>5문제 중 3문제 이상 맞추면 수료!</p>
          <button class="btn btn-primary" onclick="startEducationQuiz('${content.id}')" style="padding:13px 32px; font-size:1rem; width:100%;">
            🎯 퀴즈 시작하기
          </button>
        </div>
      </div>`;
...
/**
 * 퀴즈 전용 화면 렌더링 (본문 숨김 — 컨닝 방지)
 */
function startEducationQuiz(contentId) {
  const content = EducationService.getById(contentId);
  if (!content || !content.quiz) return;

  const quizHtml = content.quiz.map((q, qi) => `
    <div style="margin-bottom:20px; padding:16px; background:var(--color-bg-warm); border-radius:12px;">
      <p style="font-weight:700; margin-bottom:12px; font-size:0.95rem;">Q${qi + 1}. ${q.question}</p>
      <div style="display:grid; gap:8px;">
        ${q.options.map((opt, oi) => `
          <label id="quiz-opt-${qi}-${oi}" style="display:flex; align-items:center; gap:10px; padding:10px 14px; background:#fff; border:2px solid var(--color-border, #e5e5e5); border-radius:10px; cursor:pointer; transition:all 0.2s; font-size:0.9rem;" onclick="selectQuizOption(${qi}, ${oi})">
            <input type="radio" name="quiz-${qi}" value="${oi}" style="display:none;">
            <span class="quiz-radio" style="width:20px; height:20px; border:2px solid #ccc; border-radius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:all 0.2s; background:transparent;"></span>
            <span>${opt}</span>
          </label>
        `).join('')}
      </div>
  ...
  renderPage(`
    <div style="max-width:600px; margin:0 auto;">
      <button class="btn btn-secondary btn-sm" onclick="Router.navigate('/education/${contentId}')" style="margin-bottom:16px;">← 내용으로 돌아가기</button>
      <div style="text-align:center; padding:16px 0 24px;">
        <div style="font-size:1.4rem; font-weight:800; margin-bottom:4px;">${content.title}</div>
        <div style="font-size:0.85rem; color:var(--color-text-muted);">📝 5문제 중 3문제 이상 맞추면 수료!</div>
      </div>
      <div id="edu-quiz-questions">
        ${quizHtml}
      </div>
      <div id="quiz-result" style="display:none; margin-bottom:16px;"></div>
      <button class="btn btn-primary" id="quiz-submit-btn" onclick="submitEducationQuiz('${contentId}')" style="width:100%; padding:14px; font-size:1rem;">
        🎯 정답 확인하기
      </button>
    </div>
  `);
