...
const COMMUNITY_STORIES_KEY = 'communityStories';
const LEGACY_STORIES_KEY = 'stories';
let _communityStoriesMigrated = false;

function getStories() {
  let stories = StorageService.get(COMMUNITY_STORIES_KEY, []);
  if (!Array.isArray(stories)) stories = [];

  if (!_communityStoriesMigrated) {
    _communityStoriesMigrated = true;
    const legacyStories = StorageService.get(LEGACY_STORIES_KEY, []);
    if (Array.isArray(legacyStories) && legacyStories.length) {
      const seen = new Set(stories.map(s => s.id));
      const merged = stories.slice();
      ...
      if (merged.length !== stories.length) {
        stories = merged;
        saveStories(stories);
      }
    }
...
function saveStories(stories) {
  StorageService.set(COMMUNITY_STORIES_KEY, stories);
}
...
function ensureCommunitySampleStories() {
  const stories = getStories();
  const now = Date.now();
  const hasActiveSample = stories.some(s => String(s.authorId || '').startsWith('sample-') && s.expiresAt > now);
  if (hasActiveSample) return;

  const stamp = new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const samples = [
    { authorId: 'sample-1', authorName: '초코맘', authorProfileImage: '/photos/01_interior.png', imageData: '/photos/01_interior.png', text: '한강 산책로 기록' },
    { authorId: 'sample-4', authorName: '시바집사', authorProfileImage: '/images/dog_walker.png', imageData: '/images/dog_walker.png', text: '오후 산책 체크인' },
    { authorId: 'sample-5', authorName: '댕댕이네', authorProfileImage: '/photos/03_customers.png', imageData: '/photos/03_customers.png', text: '새 놀이터 저장' }
  ];

  samples.forEach((sample, index) => {
    stories.push({
      id: `sample-story-${stamp}-${index}`,
      ...sample,
      createdAt: new Date(now - (index + 1) * 42 * 60 * 1000).toISOString(),
      expiresAt: now + (18 + index) * 60 * 60 * 1000,
      viewedBy: [],
      reactions: {},
      comments: []
    });
...
function renderStoryBar(user) {
  ...
  const others = groups.filter(g => {
    ...
    if (!following.includes(g.authorId) && !g.authorId.startsWith('sample-')) return false;
    // 탈퇴한 회원이면 제외
    if (!g.authorId.startsWith('sample-') && !storedUsers.find(u => u.id === g.authorId)) return false;
    return true;
  });
  const hasMyStories = !!(myGroup && myGroup.stories.length);
  const hasMyUnviewed = hasMyStories && myGroup.stories.some(s => !s.viewedBy.includes(uid));
  const myRingClass = hasMyStories
    ? (hasMyUnviewed ? 'story-ring story-ring--active' : 'story-ring story-ring--viewed')
    : 'story-ring story-ring--add';

  return `
    <div class="story-bar story-bar--social" aria-label="커뮤니티 스토리">
      <div class="story-item story-item--mine" onclick="${hasMyStories ? `openStoryViewer('${uid}')` : 'openStoryCreate()'}">
        <div class="${myRingClass}">
          ${renderCommunityAvatar(user.profileImage, 'story-avatar')}
          <button class="story-add-icon" onclick="event.stopPropagation();openStoryCreate()" aria-label="스토리 추가">+</button>
        </div>
