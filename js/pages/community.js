...
function _addCommentFromModal(postId, inputEl) {
 ...
 if (!text) return;
 try {
   const targetPost = CommunityService.getFeed(1).find(p => p.id === postId);
...
function handleToggleLike(postId) {
 ...
 try {
 const allFeed = CommunityService.getFeed(1);
 const targetPost = allFeed.find(p => p.id === postId);
...
function handleAddReply(postId, commentId) {
 ...
 try {
   ...
   if (listEl) {
     const updatedPost = CommunityService.getFeed(1).find(p => p.id === postId);
